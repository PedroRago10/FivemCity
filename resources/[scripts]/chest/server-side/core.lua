...
function Creative.Permissions(Name,Mode)
	...
	if Passport then
		...
		elseif Mode == "Warehouse" then
			...
			local Consult = vRP.Query("chests/GetChests",{ Name = Name, name = Name })
			if not Consult[1] then
				vRP.Query("chests/AddChests",{ Name = Name })
				Consult = vRP.Query("chests/GetChests",{ Name = Name, name = Name })
			end
			local permField = Name
			if Consult[1] then
				permField = Consult[1]["Permission"] or Consult[1]["permission"] or Name
			end
			local Split = splitString(permField)
			if Consult[1] and vRP.HasPermission(tostring(Passport), Split[1], Split[2]) then
				local slots = parseInt((Consult[1]["Slots"] or Consult[1]["slots"] or 0))
				local weight = parseInt((Consult[1]["Weight"] or Consult[1]["weight"] or 0))
				local logs = (Consult[1]["Logs"] or Consult[1]["logs"] or false)
				Open[Passport] = {
					["Slots"] = math.max(slots,100),
					["Weight"] = math.max(weight,50),
					["NameLogs"] = Name,
					["Name"] = "Chest:"..Name,
					["Logs"] = logs,
					["Permission"] = permField,
					["Save"] = true
				}
				return true
			end
