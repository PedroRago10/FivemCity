                        ...
                        ["body"] = {
                            {
                                ...
                                ["items"] = {
                                    {
                                        ["type"] = "TextBlock",
                                        ["text"] = "Bem-vindo a 011RP Sao Paulo",
                                        ["weight"] = 'bolder',
                                        ["size"] = 'extraLarge', 
                                    },
                                    --[[ {
                                        ["type"] = "TextBlock",
                                        ["text"] = "Ola, "..name.."! Prepare-se para mergulhar na vida vibrante e imprevisivel da 011RP Sao Paulo! Aqui, cada escolha molda o seu destino e cada acao tem uma consequencia. Junte-se a outros jogadores, estabeleca aliancas, enfrente adversarios e construa sua propria historia neste vasto universo roleplay. Lembre-se: respeite as regras, entre no personagem e divirta-se!",
                                        ["isSubtle"] = true,
                                        ["wrap"] = true,
                                    }, ]]
                                    {
                                        ...
                                        ["actions"] = {
                                            {
                                                ["type"] = "Action.OpenUrl",
                                                ["id"] = 'copy_to_discord',
                                                ["title"] = 'https://discord.gg/SEU_LINK',
                                                ["url"] = 'https://discord.gg/SEU_LINK',
                                                ["iconUrl"] = 'https://cdn.discordapp.com/attachments/1128809150508449934/1159211818854658108/discord.png'
                                            },
                                        },
                                    },
...
function vRP.CharacterChosen(source,Passport,Model)
    ...
    if not Characters[source] then
        ...
        if GlobalGroups then
            print("DEBUG: CharacterChosen - Groups loaded. Checking permissions for Passport: " .. Passport)
            for Permission,Info in pairs(GlobalGroups) do
                ...
                if Data and Data[tostring(Passport)] then
                    print("DEBUG: CharacterChosen - Found permission: " .. Permission)
                    vRP.ServiceEnter(source,tostring(Passport),Permission,true)
                    
                    -- Explicitly force Admin state if the user has Admin group
                    if Permission == "Admin" then
                        print("DEBUG: CharacterChosen - FORCING ADMIN STATE for Source: " .. source)
                        Player(source)["state"]["Admin"] = true
                        TriggerClientEvent("Notify", source, "verde", "Admin Mode Activated", 5000)
                    end
                end
            end
        else
            print("DEBUG: CharacterChosen - Groups table is NIL")
        end
    end
